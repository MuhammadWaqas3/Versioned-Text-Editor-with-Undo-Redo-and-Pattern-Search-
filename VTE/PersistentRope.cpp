...
using namespace std;
const size_t PersistentRope::MAX_LEAF_SIZE;
...
shared_ptr<RopeNode> PersistentRope::concatenate(shared_ptr<RopeNode> left, shared_ptr<RopeNode> right) const {
    if (!left) return right;
    if (!right) return left;
...
pair<shared_ptr<RopeNode>, shared_ptr<RopeNode>> PersistentRope::split(shared_ptr<RopeNode> node, size_t pos) const {
    if (!node || pos == 0) {
        return make_pair(shared_ptr<RopeNode>(nullptr), node);
    }
    
    if (pos >= node->totalWeight()) {
        return make_pair(node, shared_ptr<RopeNode>(nullptr));
    }
    ...
    if (pos < node->weight) {
        // C++11 compatible way - no structured bindings
        pair<shared_ptr<RopeNode>, shared_ptr<RopeNode>> splitResult = split(node->left, pos);
        shared_ptr<RopeNode> leftLeft = splitResult.first;
        shared_ptr<RopeNode> leftRight = splitResult.second;
        
        auto newRight = concatenate(leftRight, node->right);
        return make_pair(leftLeft, newRight);
    } else {
        // C++11 compatible way - no structured bindings
        pair<shared_ptr<RopeNode>, shared_ptr<RopeNode>> splitResult = split(node->right, pos - node->weight);
        shared_ptr<RopeNode> rightLeft = splitResult.first;
        shared_ptr<RopeNode> rightRight = splitResult.second;
        
        auto newLeft = concatenate(node->left, rightLeft);
        return make_pair(newLeft, rightRight);
    }
}
...
shared_ptr<RopeNode> PersistentRope::rebalance(shared_ptr<RopeNode> node) const {
    if (!node || node->isLeaf()) return node;
...
PersistentRope PersistentRope::insert(size_t pos, const string& text) const {
    ...
    // C++11 compatible way
    pair<shared_ptr<RopeNode>, shared_ptr<RopeNode>> splitResult = split(root, pos);
    shared_ptr<RopeNode> left = splitResult.first;
    shared_ptr<RopeNode> right = splitResult.second;
...
PersistentRope PersistentRope::deleteText(size_t pos, size_t length) const {
    ...
    // C++11 compatible way
    pair<shared_ptr<RopeNode>, shared_ptr<RopeNode>> firstSplit = split(root, pos);
    shared_ptr<RopeNode> left = firstSplit.first;
    shared_ptr<RopeNode> temp = firstSplit.second;
    
    pair<shared_ptr<RopeNode>, shared_ptr<RopeNode>> secondSplit = split(temp, length);
    shared_ptr<RopeNode> middle = secondSplit.first;
    shared_ptr<RopeNode> right = secondSplit.second;
